...
5. **NO dejes que `pyarrow` suba a 25.x** (está pineado a `24.0.0` en `requirements.txt`). La 25.0.0 provoca `Segmentation fault` → "Oh no. Error running app" bajo el Python 3.14 de Cloud. Cualquier cambio en `requirements.txt` re-resuelve dependencias y puede volver a subir librerías nativas (pyarrow, etc.); revisa el **log de build** (Manage app → logs) tras cada deploy. La pista de un segfault es la línea `run-streamlit.sh: ... Segmentation fault`, no un traceback de Python.
