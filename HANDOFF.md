...
5. **Librerías nativas pineadas** en `requirements.txt`: `pyarrow==24.0.0` y `websockets==16.0`. El set que funcionaba en Cloud (Python 3.14) era `pandas 3.0.3 + pyarrow 24.0.0 + websockets 16.0`. Un rebuild subió pyarrow→25.0.0 y websockets→16.1 y la app empezó a caer con `Segmentation fault` → "Oh no. Error running app". Cualquier cambio en `requirements.txt` re-resuelve dependencias y puede volver a subir librerías nativas; revisa el **log de build** (Manage app → logs) tras cada deploy. La pista de un segfault es la línea `run-streamlit.sh: ... Segmentation fault`, no un traceback de Python. (pandas 3.0 NO es el problema; corría bien.)
